...
botaoCalcular.addEventListener("click", function (event) {
  ...
  if (!isNaN(populacaoFixa) && tipoOcupacao && ufSelecionada) {
    ...
    exibirResultado(resultado, ufSelecionada, populacaoFixa, inputPublico, nomeInstituicao, tipoOcupacao);
  ...
  function exibirResultado(
    ...
    populacaoFixa,
    inputPublico,
    nomeArea, tipoOcupacao
  ) {
    resultadoDiv.innerHTML = `<p><center><strong>${ufSelecionada}</strong></center></p><p>Nome da Área/Instituição: ${nomeArea}</p><p>${tipoOcupacao}</p><p>População Fixa: ${populacaoFixa}</p><p>Público (se houver): ${inputPublico}</p><p><strong>Total de Brigadistas: ${resultado.totalBrigadistas}</strong></p><p>${resultado.texto}</p>`;
  }
  ...
  function calcularBrigada(populacaoFixa, parametros, tipoOcupacao, inputPublico) {
    console.log("tipoOcupacao:", tipoOcupacao);
    console.log("inputPublico:", inputPublico);

    const [porcentagem10, porcentagemMais10, texto] = parametros;

    // Verifica se o tipo de ocupação é "F-7 Construção provisória e evento temporário"
    if (tipoOcupacao === "F-7 Construção provisória e evento temporário") {
        console.log("Entrou no if F-7");
        const textoEspecial = parametros[2];
        let totalBrigadistas;
        if (inputPublico < 500) {
            // Se o público for menor que 500, o número mínimo de brigadistas é 2
            totalBrigadistas = 2;
        } else {
            // Calcula o número de brigadistas com base na população e arredonda para cima
            totalBrigadistas = Math.ceil(inputPublico / 500);
        }
        console.log("totalBrigadistas:", totalBrigadistas);
        return { totalBrigadistas, texto: textoEspecial };
    }

    // Calcula a brigada de incêndio para outros tipos de ocupação
